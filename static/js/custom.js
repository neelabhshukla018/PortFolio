...
const text = "𝔽𝕣𝕠𝕟𝕥-𝔼𝕟𝕕-𝕎𝕖𝕓-𝔻𝕖𝕧𝕖𝕝𝕠𝕡𝕖𝕣";
  const typeIt = document.getElementById("type-it");
  ...
  function typeAnimation() {
    if (forward) {
      typeIt.textContent = text.slice(0, index++);
    ...
    setTimeout(typeAnimation, 120); // speed
  }
...
let resumeFrame = document.getElementById("resumeFrame");
let zoomLevel = 1;
...
function zoomIn() {
  zoomLevel += 0.2;
  resumeFrame.style.transform = `scale(${zoomLevel})`;
}
function zoomOut() {
  if (zoomLevel > 0.4) {
    zoomLevel -= 0.2;
...
function downloadResume() {
  ...
  link.href = "your-resume.pdf"; // replace with your resume file
...
function printResume() {
  let frame = document.getElementById("resumeFrame").contentWindow;
  frame.focus();
  frame.print();
}
